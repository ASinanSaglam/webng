...
if __name__ == '__main__':
    ...
    mapper = ass.load_mapper('../west.h5', 300)
    X = mapper.centers[:,0]
    Y = mapper.centers[:,1]
    ...
    plt.savefig('exMISA_voronoi.png')
    plt.close()
    for i in range(1,mapper.centers.shape[0]):
        X = mapper.centers[:i,0]
        Y = mapper.centers[:i,1]
        fig = plt.figure(figsize=(10,10))
        axes = plt.subplot(1,1,1)
        plt.scatter(X,Y)
        segments = voronoi(X,Y)
        lines = matplotlib.collections.LineCollection(segments, color='0.75')
        axes.add_collection(lines)
        plt.savefig('voronoi_bins/exMISA_voronoi_%04d.png'%i)
        plt.close()
